...
Proof.
  ...
  injection eq1 as H1 H2. rewrite <- H2. 
  intros eq2. 
  injection eq2 as H3.
...
Proof.
  ...
  - intros m eq. 
    destruct m as [| m'] eqn:H. 
    + reflexivity.
    + discriminate.
  - intros m eq.
    destruct m as [| m'] eqn: H.
    + discriminate.
    + f_equal. simpl in eq. apply IHn'. apply eq.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. 
    destruct m as [|m'] eqn: M.
    + discriminate.
    + simpl in H. injection H as H2. 
      rewrite <- plus_n_Sm in H2. 
      rewrite <- plus_n_Sm in H2. 
      injection H2 as H. apply IHn' in H.
      f_equal. apply H.
Qed.
...
Proof.
  intros X Y l.
  induction l as [|lh lt IHl].
  - intros. simpl in H. injection H as H1 H2. rewrite <- H1. rewrite <- H2. simpl. reflexivity.
  - intros. simpl in H. destruct lh. destruct (split lt). injection H as H1 H2. 
    rewrite <- H1. rewrite <- H2. simpl. f_equal. rewrite IHl. 
    + reflexivity. 
    + reflexivity.
Qed.
(** [] *)
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros n m p eq1 eq2. generalize dependent m.
  induction n as [|n' IHn'].
  - intros. apply eqb_true in eq1. rewrite eq1. apply eq2.
  - intros. apply eqb_true in eq1. rewrite eq1. apply eq2.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. simpl in IHl. simpl in H. destruct (test lh) eqn:T.
    + injection H as H1 H2. rewrite H1 in T. apply T.
